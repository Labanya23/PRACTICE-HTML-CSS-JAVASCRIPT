...
window.onload=function ()
{
...

    {
        let task=
        {
            task:inp.ariaValueMax,
            done:false
        }
        globalThis.todolist.push(task);
        ...
        for(let i=0;i<global.todolist.length;i++)
            {
                ...
                <button id="b${i}"onclick=deleteEL(this)" style="background-color:darkgreen">DELETE</button></li>`;
...
function toggle(e1)
{
    ...
    strike0ff(checked,id);
    loacalStorage.setItem('todolist',JSON.stringify(global.todolist));

}
function strike0ff(check,id)
{
